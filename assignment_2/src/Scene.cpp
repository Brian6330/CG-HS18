...
vec3 Scene::lighting(const vec3& _point, const vec3& _normal, const vec3& _view, const Material& _material)
{
	...
	*/
	...
	const vec3 ambient_component = ambience * _material.ambient;

	vec3 diffuse_component = vec3(0, 0, 0);
	vec3 specular_component = vec3(0, 0, 0);

	for (const Light &l : lights) {
		const vec3 light_dir = _point - l.position;

		// diffuse component
		diffuse_component += l.color * _material.diffuse * std::max(0.0, dot(_normal, light_dir));

		// specular component
		const vec3 reflection = reflect(light_dir, _normal);
		const double shine = pow(std::max(0.0, dot(reflection, _view)), _material.shininess);
		specular_component += l.color * _material.specular * shine;
	}



	vec3 true_color = ambient_component + diffuse_component + specular_component;
